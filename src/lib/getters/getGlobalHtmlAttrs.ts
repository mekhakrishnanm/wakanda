...
type CommonAttrs = Pick<
  AllHTMLAttributes<any>,
  (typeof microDataAttrs)[number]
> &
...
type Attrs = {
  HTMLInputElement: Pick<
    InputHTMLAttributes<HTMLInputElement>,
    (typeof inputAttrs)[number]
  >;
  HTMLSelectElement: Pick<
    SelectHTMLAttributes<HTMLSelectElement>,
    (typeof selectAttrs)[number]
  >;
  HTMLTextAreaElement: Pick<
    TextareaHTMLAttributes<HTMLTextAreaElement>,
    (typeof textareaAttrs)[number]
  >;
  // biome-ignore lint/complexity/noBannedTypes: <explanation>
  default: {};
};

export type GlobalHTMLAttrs<T extends keyof Attrs = 'default'> = CommonAttrs &
  Attrs[T];
...
const getGlobalHtmlAttrs = <T = GlobalHTMLAttrs>(props: Props) =>
  Object.keys(props).reduce((attrs, key) => {
    if (
      ...
      attrs[key as keyof T] = props[key];
    }
